...
- BPM histogram, key pie, mood averages (pie uses **primary** genre folders). Click a BPM bucket or key (chart or table) to list matching recordings
...
- `All recordings in library` opens `/Recent` filtered to that Essentia label, BPM range, or key
...
- `GetAudioLabelDetail(kind, name, TimeRange)` — recordings for one Essentia genre/theme/instrument (folder prefix for a primary genre), BPM bucket, or key
